...
object_key_state:
  if (parser.advance_char() != ':' ) { parser.log_error("Missing colon after key in object"); return TAPE_ERROR; }
  switch (parser.advance_char()) {
...
object_continue:
  ...
  case ',':
    parser.increment_count();
    if (parser.advance_char() != '"' ) { parser.log_error("Key string missing at beginning of field in object"); return TAPE_ERROR; }
    SIMDJSON_TRY( parser.parse_string(true) );
    goto object_key_state;
  case '}':
    parser.end_object();
